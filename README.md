...
代码与测试核对日期：2026-04-19。端到端复测记录见 `docs/reviews/CR-CODEX-E2E-2026-04-17.md`，模型矩阵见：
...
| 上游断流可终止性 | 通过 | 通过 | 遇到上游空流/异常流时，现会显式返回 `response.completed` + `Codex adapter error`，不再无限重连 |
| 真实复杂任务 | 不通过 | 不通过 | 最新 6 模型复测已进入真实 `command_execution`（`1~6` 次），但仍未稳定完成“双读取 + 收束答案”闭环 |
| `spawn_agent` | 不通过 | 不通过 | 仍依赖稳定工具状态机，当前上游模型在真实任务中无法持续遵守 |
| `kimi-k2p5` 最新复测 | 不通过 | 不通过 | 直连 `iter9`：`cmd_count=3`；中转 `iter4`：`cmd_count=3`；两链路都未完成“读取两处 + 三行收束”闭环 |
| 6 模型中转复测（2026-04-18） | - | 不通过 | postpatch 后 `commands_ok=12/12`、`turn.completed=12/12`、`adapter_error=0/12`，但严格三行收束仍仅 `2/12`（证据：`/private/tmp/codex-iter-matrix-20260418-230952-postpatch/summary_classified.tsv`） |
| 6 模型双链路复测（2026-04-19） | 不通过 | 不通过 | 本轮启用“标签硬门禁 + 伪工具标记拦截 + 空流重试”，直连 `6/6` 虽完成命令但最终均被门禁改写为 `adapter error`；中转仅 `glm-5` 完成，其他 `5/6` 在 Codex 侧高负载重连后 `turn.failed`（证据：`/private/tmp/codex-iter-matrix-20260419-005535-postopt2/summary_classified.tsv`） |
...
- 修复前矩阵（`/private/tmp/codex-iter-matrix-20260418-224926-postfix/summary_classified.tsv`）：`commands_ok=10/12`、`turn.completed=10/12`、`adapter_error=2/12`。
- 修复后矩阵（`/private/tmp/codex-iter-matrix-20260418-230952-postpatch/summary_classified.tsv`）：`commands_ok=12/12`、`turn.completed=12/12`、`adapter_error=0/12`。
- `execution_stage=finalize` 已不再出现工具循环（`tools_present=false`，且无 `finalize` 阶段 `tool protocol outcome`）。
- 补充抽测（`/private/tmp/codex-iter-retest-20260418-230719-postfix2`）观察到 `deepseek-v3p2` 偶发上游 `tls: bad record MAC`，但客户端会收到可终止的 `response.completed` 错误消息，不再卡在重连环。
- 通过数据库复核，2026-04-18 相关中转请求均命中 `channel_id=106`，中转失败不是渠道漂移导致。
- 但严格三行收束与事实一致性仍不稳定，复杂 Agent 任务结论仍为“不具备生产可用性”。

2026-04-19 新一轮复测（证据：`/private/tmp/codex-iter-matrix-20260419-005535-postopt2`）：

- 本轮 `responses` 增加了 `EXECUTION_EVIDENCE` 注入、最终标签硬校验、伪工具标记拦截与空流一次重试。
- 直连链路：`commands_ok=6/6`、`turn.completed=6/6`，但最终输出 `adapter_error=6/6`（模型未稳定满足 `RESULT/README/TOOLP` 收束格式，或泄漏伪工具控制标记）。
- 中转链路：仅 `glm-5` 成功闭环并满足三行格式；其余 5 模型均出现 `Reconnecting... high demand` 并 `turn.failed`。
- 路由复核显示：多数请求仍命中 `channel_id=106`；`glm-5` 存在少量落到 `channel_id=72/75` 的重试记录。

...
- `/var/folders/hq/q19jry150l16mrrbkh7wm0_m0000gn/T/firew2oai-realchain-matrix-20260425-133952/summary.tsv`
...
- `qwen3-vl-30b-a3b-thinking`: `stdin_read_loop_timeout`

通过样本均完成 `mcp__docfork__search_docs -> mcp__docfork__fetch_doc -> exec_command README.md -> RESULT: PASS`。剩余失败样本已有 Docfork 工具进展，但未完成 README 读取与最终结构化收口。

## 结论

本轮问题主要是适配层和评测口径缺口，不是 Docfork 工具本身不可用。修复后 Docfork 场景已从 `0/12 PASS` 收敛为 `11/12 PASS`，剩余风险集中在 `qwen3-vl-30b-a3b-thinking` 的长尾循环与收口稳定性。
